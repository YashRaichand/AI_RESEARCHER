...
async def create_session(
    ...
    return {"id": str(session.id), "title": session.title, "paper_ids": session.paper_ids, "created_at": str(session.created_at)}
...
async def list_sessions(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return [{"id": str(s.id), "title": s.title, "paper_ids": s.paper_ids, "created_at": str(s.created_at)} for s in sessions]
...
async def send_message(
    ...
    history_result = await db.execute(
        select(ChatMessage).where(ChatMessage.session_id == session_id).order_by(ChatMessage.created_at.desc()).limit(10)
    )
    history = [{"role": m.role, "content": m.content} for m in reversed(history_result.scalars().all())]

    user_msg = ChatMessage(id=uuid.uuid4(), session_id=session.id, role=MessageRole.user, content=data.question, citations=[])
    db.add(user_msg)
    await db.commit()
    ...
    async def stream_response():
        ...
        citations = []
        try:
            async for chunk in _chat_engine.chat(data.question, paper_ids=session.paper_ids, chat_history=history):
                if "__CITATIONS__" in chunk:
                    text_part, citations = _chat_engine.extract_citations_from_response(chunk)
                    if text_part:
            ...
            async with AsyncSessionLocal() as save_db:
                assistant_msg = ChatMessage(
                    ...
                    role=MessageRole.assistant, content=full_response, citations=citations,
                )
...
async def get_history(session_id: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    result = await db.execute(select(ChatSession).where(ChatSession.id == session_id, ChatSession.user_id == current_user.id))
    session = result.scalar_one_or_none()
    if not session:
        raise HTTPException(404, "Session not found")
    ...
    return {"session_id": session_id, "messages": [{"id": str(m.id), "role": m.role, "content": m.content, "citations": m.citations, "created_at": str(m.created_at)} for m in messages]}
